...
void Playlist::clear() {
    ...
    while (current != nullptr)
    {
        ...
        delete current->track; // because only clones are passed to add_track
        delete current;
...
Playlist::Playlist(const Playlist &p) : head(nullptr), playlist_name(p.playlist_name), track_count(0)
{  
    PlaylistNode *current = p.head;
    while (current != nullptr)
    {
        PointerWrapper<AudioTrack> clone = current->track->clone();
        this->add_track(clone.release());
