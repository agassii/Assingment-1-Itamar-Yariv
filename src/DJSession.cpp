...
int DJSession::load_track_to_controller(const std::string& track_name) {
 ...
 AudioTrack* track = library_service.findTrack(track_name);
    ...
    int ans = controller_service.loadTrackToCache(*track);
    if(ans ==1){
        stats.cache_hits++;
    }
    else{
...
bool DJSession::load_track_to_mixer_deck(const std::string& track_title) {
    ...
    // your implementation here
 AudioTrack* cache_track = controller_service.getTrackFromCache(track_title);
    if(!cache_track){
        std::cout<<  "[ERROR] Track: \"" << track_title<< "\" not found in cache" << std::endl;
        stats.errors++;
        return false;
    }
    int deck_index = mixing_service.loadTrackToDeck(*cache_track);
    if (deck_index == -1){
     std::cout << "[ERROR] Failed to load track: \"" << track_title<< "\" to deck" << std::endl;
       ...
       std::vector<std::string> playlist_names;
        if(play_all){
              for (const auto& kv : session_config.playlists) {
                playlist_names.push_back(kv.first);
            }
            ...
            playlist_names.push_back(chosen);
        
        }
        for (const std::string& playlist_name : playlist_names) {
        bool loaded = load_playlist(playlist_name);
        if(!loaded){
